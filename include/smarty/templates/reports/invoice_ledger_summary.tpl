    ...
    <table width="100%" class="mapcasting_tabelle" border="0" cellspacing="0" cellpadding="2"
        ...
        <tr>
            <td valign="middle" align="left" colspan="4">
                ...
                [&nbsp;<a title="{$PrintMsg} - {$SearchResult} - {$DateFrom} {$Date_Till} {$DateTill}"
                          class="ninfolink"
                          href="pdf/print_pdf.php?DateFrom={$DateFrom}&amp;DateTill={$DateTill}&amp;Type=Invoice_Ledger_Summary&amp;Order={$Order}&amp;Sort={$Sort}&amp;{$Session}"
        ...
        <tr class="mblueTD">
            ...
            <td nowrap="nowrap" align="right" width="20%">{$Invoice_Amount} {$Invoice_Currency}
                <a href="{$smarty.server.PHP_SELF}?{$AddCurrentPage}Order=TOTAL_AMOUNT&amp;Sort=ASC&amp;DateFrom={$DateFrom}&amp;DateTill={$DateTill}&amp;Report={$Report}&amp;{$Session}"><img
                        border="0" src="../images/up.png" title="{$SortMsg} {$Invoice_Amount} ASC"
                        alt="{$SortMsg} {$Invoice_Amount} ASC"/></a>
                <a href="{$smarty.server.PHP_SELF}?{$AddCurrentPage}Order=TOTAL_AMOUNT&amp;Sort=DESC&amp;DateFrom={$DateFrom}&amp;DateTill={$DateTill}&amp;Report={$Report}&amp;{$Session}"><img
            ...
            <td nowrap="nowrap" align="right" width="20%">{$Open_Account}
                <a href="{$smarty.server.PHP_SELF}?{$AddCurrentPage}Order=OPEN_ACCOUNT&amp;Sort=ASC&amp;DateFrom={$DateFrom}&amp;DateTill={$DateTill}&amp;Report={$Report}&amp;{$Session}"><img
                        border="0" src="../images/up.png" title="{$SortMsg} {$Open_Account} ASC"
                        alt="{$SortMsg} {$Open_Account} ASC"/></a>
                <a href="{$smarty.server.PHP_SELF}?{$AddCurrentPage}Order=OPEN_ACCOUNT&amp;Sort=DESC&amp;DateFrom={$DateFrom}&amp;DateTill={$DateTill}&amp;Report={$Report}&amp;{$Session}"><img
        ...
        <td valign="top" align="left" width="50%"><a
                title="{$AllInformation} {$Customer_No} {$invoice.MYID} -{if $invoice.FIRSTNAME neq " "}
            {$invoice.FIRSTNAME}{/if}{if $invoice.LASTNAME neq " "} {$invoice.LASTNAME},{/if}{if
            $invoice.COMPANY} {$invoice.COMPANY}{/if}" class="ninfolink"
            href="../addressbook/info.php?myID={$invoice.MYID}&amp;infoID=30&amp;{$Session}" target="_blank">
            {$invoice.MYID} -
            ...
            {$invoice.COMPANY}
            {/if}</a></td>
        <td nowrap="nowrap" valign="top" align="right" width="20%">{$invoice.TOTAL_AMOUNT|number_format}
            ({$invoice.TOTAL_AMOUNT/$TOTAL_AMOUNT*100|number_format}&#037;)
        </td>
        <td nowrap="nowrap" valign="top" align="right" class="redtxt" width="20%">{if $invoice.OPEN_ACCOUNT neq
            0}{$invoice.OPEN_ACCOUNT|number_format}{/if}
        </td>
        ...
        <td>
            ...
            {foreachelse}
            <tr>
                <td align="center" colspan="4" class="redtxt">{$NoEntry}</td>
            </tr>
            {/foreach}
            <tr>
                <td>&nbsp;</td>
            ...
            <tr>
                <td valign="top" align="right" colspan="3">{$Open_Account} {$Invoice_Currency}:</td>
                <td valign="top" align="right" class="redtxt">{$OPEN_ACCOUNT|number_format}</td>
            </tr>
            {/if}
            <tr>
                <td valign="top" align="right" colspan="3">{$Invoice_Amount} - {$PageMsg} {$Invoice_Currency}:
                </td>
                <td valign="top" align="right">{$TOTAL_PAGE|number_format}</td>
            </tr>
            <tr>
                <td valign="top" align="right" colspan="3"><b>{$Invoice_Amount} {$Searchresult}
                    {$Invoice_Currency}</b>:
                </td>
            ...
            {* Include the linkbar *}
            <tr>
                <td colspan="4">{include file="linkbar.tpl"}</td>
            </tr>
            {/if}
        </tbody>
    </table>
