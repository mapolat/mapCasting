    ...
    <tr>
        ...
        <td width="78%" valign="top" align="center">
            <table width="80%" border="0" cellpadding="2" cellspacing="0" summary="Tabelle 1">
                <tr>
                    <td class="mapcasting_tabelle">
                        <table width="100%" border="0" cellspacing="3" cellpadding="3" summary="Tabelle 2">
                            <tr>
                                <td align="center" colspan="2"><h2>{$Reports}</h2></td>
                            </tr>
                            <tr>
                                <td align="right" width="40%" valign="top">{$Reports}:</td>
                                {* Display all available reports *}
                                <td align="left" width="60%" valign="middle">
                                    <form id="Report" name="Report" action="{$smarty.server.PHP_SELF}?{$Session}"
                                          method="post">
                                        <input type="hidden" name="DateFrom" value="{$DateFrom}"/>
                                        <input type="hidden" name="DateTill" value="{$DateTill}"/>
                                        <select class="choice250" name="Report" title="{$Select_Report}"
                                                onChange="this.form.submit();">
                                            <optgroup label="{$Reports}" title="{$Reports}">
                                                <option label="{$Select_Report}" title="{$Select_Report}" value="">---
                                                    {$Select_Report} ---
                                                </option>
                                                {foreach item=report from=$choose_report}
                                                {foreach key=key item=item from=$report}
                                                {if $Report eq $key}
                                                <option label="{$item}" title="{$item}" value="{$key}"
                            ...
                            <form id="NewReport" name="NewReport" action="{$Report}?{$Session}" method="post">
                                ...
                                <tr>
                                    <td align="right" width="40%" valign="middle">{$DateMsg} {$Date_From}:</td>
                                    <td align="left" width="60%" valign="middle"><input title="{$DateMsg} {$Date_From}"
                                                                                        class="form_input" type="text"
                                                                                        name="DateFrom" size="30"
                                                                                        value="{if $DateFrom}{$DateFrom}{else}{$smarty.now|date_format:"%d.%m.%Y"}{/if}"
                                        />
                                    </td>
                                </tr>
                                <tr>
                                    <td align="right" width="40%" valign="middle">{$DateMsg} {$Date_Till}:</td>
                                    <td align="left" width="60%" valign="middle"><input title="{$DateMsg} {$Date_Till}"
                                                                                        class="form_input" type="text"
                                                                                        name="DateTill" size="30"
                                                                                        value="{if $DateTill}{$DateTill}{else}{$smarty.now|date_format:"%d.%m.%Y"}{/if}"
                                        />
                                    </td>
                                </tr>
                                <tr>
                                    <td align="center" colspan="2"><input class="button" type="submit" title="{$Search}"
                                                                          value="{$Search}"/></td>
                                </tr>
                            </form>
                            {/if}
